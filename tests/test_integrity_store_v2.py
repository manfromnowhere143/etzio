...
_V3_ONLY_TRIGGERS = (
    ...
    "integrity_blocked_reject_finalized",
    "integrity_blocked_reject_update",
    "integrity_recovery_decision_reject_delete",
    "integrity_recovery_decision_reject_update",
    "integrity_recovery_profile_reject_delete",
    "integrity_recovery_profile_reject_update",
    "integrity_recovery_reject_after_seal",
    "integrity_recovery_require_latest_observation",
)
_V3_ONLY_INDEXES = ("integrity_blocked_latest_attempt",)
_V3_ONLY_TABLES_IN_DROP_ORDER = (
...
def _downgrade_exact_empty_integrity_schema_to_v2(path: Path) -> None:
    ...
    try:
        for name in _V3_ONLY_TRIGGERS:
            connection.execute(f'DROP TRIGGER "{name}"')
        for name in _V3_ONLY_INDEXES:
            connection.execute(f'DROP INDEX "{name}"')
        for name in _V3_ONLY_TABLES_IN_DROP_ORDER:
            connection.execute(f'DROP TABLE "{name}"')
        connection.execute(f"PRAGMA user_version = {_LEGACY_INTEGRITY_SCHEMA_VERSION}")
    finally:
        connection.close()
